...
public class CharMovement : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        if (((!isCrouched && isGrounded) || !isGrounded) && !isShielding && inputActive) 
        {
        ...
        if (dashBuffer && !isWalking && dash > 0 && dash <= 0.40 && dashQueue.Count >= 2)
        {
        ...
        if (dashBuffer && ((!facingRight && moveHorizontal < 0) || (facingRight && moveHorizontal > 0)))
        {
            dash += Time.deltaTime;
        }

        else if (dashBuffer && ((!facingRight && moveHorizontal > 0) || (facingRight && moveHorizontal < 0)))
        {
            dash += Time.deltaTime;
            backDash += Time.deltaTime;
        }
        ...
        if ((dash > 0.40f && !isDashing) || backDash > 0.60f)
        {
            isWalking = true;
            dash = 0;
            backDash = 0;
        ...
        {
            speed = 5f;
            /*Try and catch not needed 
             * the logic is designed to prevent it from ever being called when not valid
             * making the try/catch redundant clutter*/
